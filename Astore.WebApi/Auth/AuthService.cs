...
public class AuthService : IAuthService
{
    ...
    public AuthService(UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager,
        StoreDbContext dbContext, JwtSettings jwtSettings)
    {
    ...
    public async Task<AuthResult> LoginAsync(string email, string password)
    {
        ...
        if (user == null) return new AuthResult { Errors = new[] { "Invalid email or password" } };

        var isPasswordValid = await _userManager.CheckPasswordAsync(user, password);

        if (!isPasswordValid) return new AuthResult { Errors = new[] { "Invalid email or password" } };
